    ...
    while menu:
        ...
        if cloud_x <= 800 - 1100:
            ...
            if not player.wreck_start:
                cloud_x -= 5
        if godmode:
            title = message_to_screen("CYBERWARE (GODMODE)", font, 80, yellow)
        else:
            title = message_to_screen("CYBERWARE", font, 100, black)
        ...
        if selected == "infor":
            information = message_to_screen("INFOR", font, 75, white) 
        else:
            information = message_to_screen("INFOR", font, 75, white)       
        ...
        pygame.display.set_caption("CYBERWARE running at " + str(int(clock.get_fps())) + " frames per second.")
        clock.tick(FPS)
